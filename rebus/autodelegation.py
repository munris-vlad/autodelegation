...
class RebusAutodelegation():
    def __init__( self, config_file='config.ini' ):
        ...
        self.send( f'Hello from REBUS Autodelegation Bot on {self.name}!' )
    ...
    def setup_rebus_info( self ):
        ...
        elif 'sleep_time' in self.config['REBUS']:
            self.sleep_time = int(self.config['REBUS']['sleep_time'])
        else:
            self.sleep_time = 600
        
        # bank reserve
        if "REBUS_RESERVE" in os.environ:
            self.reserve = float(os.environ['REBUS_RESERVE'])
        elif 'reserve' in self.config['REBUS']:
            self.reserve = float(self.config['REBUS']['reserve'])
        else:
            self.reserve = 0.1000

        # Prompt for the password if not in environment
        if "REBUS_PASSWORD" in os.environ:
            self.password = os.environ['REBUS_PASSWORD']
        elif 'password' in self.config['REBUS']:
            self.password = self.config['REBUS']['password']
        else:
        ...
        if "CHAIN_ID" in os.environ:
            ...
            self.chain_id = self.config['REBUS']['chain_id']
        ...
        if "WALLET_NAME" in os.environ:
            ...
            self.wallet_name = self.config['REBUS']['wallet_name']
        
        # wallet and validator keys
        if "WALLET_KEY" in os.environ:
            self.wallet_key = os.environ['WALLET_KEY']
        elif 'wallet_key' in self.config['REBUS']:
            self.wallet_key = self.config['REBUS']['wallet_key']
        elif "WALLET_ADDRESS" in os.environ:
            self.wallet_key = os.environ['WALLET_ADDRESS']
        elif 'wallet_address' in self.config['REBUS']:
            self.wallet_key = self.config['REBUS']['wallet_address']
        else:
        ...
        elif 'validator_key' in self.config['REBUS']:
            self.validator_key = self.config['REBUS']['validator_key']
        elif "VALIDATOR_ADDRESS" in os.environ:
            self.validator_key = os.environ['VALIDATOR_ADDRESS']
        elif 'validator_address' in self.config['REBUS']:
            self.validator_key = self.config['REBUS']['validator_address']
        else:
    ...
    def shares_to_decimal( self, shares ):
        ...
        '''
        return float( shares ) * ( 1/REBUS_DECIMALS )

    def decimal_to_shares( self, amount ):
        '''
        return the decimal to shares conversion
        '''
        return int( amount * REBUS_DECIMALS )

    def get_balance( self ):
        '''
        Obtain the REBUS balance
        '''
        proc = Popen([ f"rebusd q bank balances {self.wallet_key}" ], stdout=PIPE, shell=True)
    ...
    def distribute_rewards( self ):
        ...
        '''
        child = pexpect.spawn(f"rebusd tx distribution withdraw-rewards { self.validator_key } --chain-id={ self.chain_id } --from {self.wallet_name} -y", timeout=10)
    ...
    def distribute_rewards_commission( self ):
        ...
        '''
        child = pexpect.spawn(f"rebusd tx distribution withdraw-rewards { self.validator_key } --chain-id={ self.chain_id } --from {self.wallet_name} --commission -y", timeout=10)
        child.expect( b'Enter keyring passphrase:' ) 
        child.sendline( self.password )   
        child.expect( pexpect.EOF )                                                                                                                                     
        child.close()
    ...
    def delegate( self, amount ):
        ...
        '''
        child = pexpect.spawn( f'rebusd tx staking delegate { self.validator_key } { amount }utorii --from { self.wallet_name } --chain-id { self.chain_id } -y', timeout=10)
        child.expect( b'Enter keyring passphrase:' ) 
        child.sendline( self.password )   
        child.expect( pexpect.EOF )                                                                                                                                     
        child.close()
    ...
    def get_delegations( self ):
        ...
        '''
        proc = Popen([ f"rebusd q staking delegations-to {self.validator_key} --chain-id={self.chain_id}" ], stdout=PIPE, shell=True)
        (out, err) = proc.communicate()
...
# Create the object
rebus_bot = RebusAutodelegation( args.config )

# run periodic delegation cycle
while True:
    rebus_bot.delegation_cycle()

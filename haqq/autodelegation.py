...
class HaqqAutodelegation():
    ...
    def setup_haqq_info( self ):
        ...
        elif 'sleep_time' in self.config['HAQQ']:
            self.sleep_time = int(self.config['HAQQ']['sleep_time'])
        else:
            self.sleep_time = 600
        
        # bank reserve
        if "HAQQ_RESERVE" in os.environ:
            self.reserve = float(os.environ['HAQQ_RESERVE'])
        elif 'reserve' in self.config['HAQQ']:
            self.reserve = float(self.config['HAQQ']['reserve'])
        else:
            self.reserve = 0.1000

        # Prompt for the password if not in environment
        if "HAQQ_PASSWORD" in os.environ:
            self.password = os.environ['HAQQ_PASSWORD']
        elif 'password' in self.config['HAQQ']:
            self.password = self.config['HAQQ']['password']
        else:
        ...
        if "CHAIN_ID" in os.environ:
            ...
            self.chain_id = self.config['HAQQ']['chain_id']
        ...
        if "WALLET_NAME" in os.environ:
            ...
            self.wallet_name = self.config['HAQQ']['wallet_name']
        
        # wallet and validator keys
        if "WALLET_KEY" in os.environ:
            self.wallet_key = os.environ['WALLET_KEY']
        elif 'wallet_key' in self.config['HAQQ']:
            self.wallet_key = self.config['HAQQ']['wallet_key']
        elif "WALLET_ADDRESS" in os.environ:
            self.wallet_key = os.environ['WALLET_ADDRESS']
        elif 'wallet_address' in self.config['HAQQ']:
            self.wallet_key = self.config['HAQQ']['wallet_address']
        else:
        ...
        elif 'validator_key' in self.config['HAQQ']:
            self.validator_key = self.config['HAQQ']['validator_key']
        elif "VALIDATOR_ADDRESS" in os.environ:
            self.validator_key = os.environ['VALIDATOR_ADDRESS']
        elif 'validator_address' in self.config['HAQQ']:
            self.validator_key = self.config['HAQQ']['validator_address']
        else:
    ...
    def shares_to_decimal( self, shares ):
        ...
        '''
        return float( shares ) * ( 1/HAQQ_DECIMALS )

    def decimal_to_shares( self, amount ):
        '''
        return the decimal to shares conversion
        '''
        return int( amount * HAQQ_DECIMALS )

    def get_balance( self ):
        '''
        Obtain the HAQQ balance
        '''
        proc = Popen([ f"haqqd q bank balances {self.wallet_key}" ], stdout=PIPE, shell=True)
    ...
    def distribute_rewards( self ):
        ...
        '''
        child = pexpect.spawn(f"haqqd tx distribution withdraw-rewards { self.validator_key } --chain-id={ self.chain_id } --from {self.wallet_name} -y", timeout=10)
    ...
    def distribute_rewards_commission( self ):
        ...
        '''
        child = pexpect.spawn(f"haqqd tx distribution withdraw-rewards { self.validator_key } --chain-id={ self.chain_id } --from {self.wallet_name} --commission -y", timeout=10)
        child.expect( b'Enter keyring passphrase:' ) 
        child.sendline( self.password )   
        child.expect( pexpect.EOF )                                                                                                                                     
        child.close()
    ...
    def delegate( self, amount ):
        ...
        '''
        child = pexpect.spawn( f'haqqd tx staking delegate { self.validator_key } { amount }aISLM --from { self.wallet_name } --chain-id { self.chain_id } -y', timeout=10)
        child.expect( b'Enter keyring passphrase:' ) 
        child.sendline( self.password )   
        child.expect( pexpect.EOF )                                                                                                                                     
        child.close()
    ...
    def get_delegations( self ):
        ...
        '''
        proc = Popen([ f"haqqd q staking delegations-to {self.validator_key} --chain-id={self.chain_id}" ], stdout=PIPE, shell=True)
        (out, err) = proc.communicate()
...
# Create the object
haqq_bot = HaqqAutodelegation( args.config )

# run periodic delegation cycle
while True:
    haqq_bot.delegation_cycle()
